...
downloads_dir = os.path.join(root_dir, "ChatBotAutomation", "gemini-downloader", "downloads_genz")
music_dir = os.path.join(root_dir, "youtube_music")
output_dir = os.path.join(root_dir, "genz_automation", "content")
os.makedirs(output_dir, exist_ok=True)

# Collect and sort images
image_files = sorted([os.path.join(downloads_dir, f) for f in os.listdir(downloads_dir) if f.lower().endswith('.png')])
...
for idx, image_path in enumerate(image_files):
    try:
        ...
        # Force height to be even to avoid FFMPEG errors
        w, h = clip.size
        if h % 2 != 0:
